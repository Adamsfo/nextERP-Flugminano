...
const Page = () => {
  ...
  const columns = [
    ...
    { key: 'login', _style: { minWidth: '200px' }, label: 'Login' },
    { key: 'nomeCompleto', _style: { minWidth: '100px' }, label: 'Nome Completo' },
    {
      key: 'funcaoUsuario_funcaoUsuario',
    ...
    { key: 'ativo', label: 'Ativo', filter: false, sorter: false },
    { key: 'show_details', label: 'Ação', _style: { width: '2%' }, filter: false, sorter: false },
  ]

  const ativo = (item: any) => {
    return <td style={{ height: '110%' }}>{item.ativo ? 'Sim' : 'Não'}</td>
  }
  ...
  const getRegistros = async (params: QueryParams) => {
    const reg = await apiGeral.getResource(endpointApi, params)
    console.log(reg)
    return reg
  }
  ...
  return (
    <PermissionGate permission={9}>
      <CRow className="g-3">
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>Usuário do Sistema</strong>
            </CCardHeader>
            <CCardBody>
              <CRow className="g-3">
                <CCol md={6}></CCol>
              ...
              <SmartTableWrapper
                ...
                columns={columns}
                scopedColumns={{ ativo, show_details }}
                search={search}
