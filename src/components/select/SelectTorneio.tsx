...
import { Blind, EstruturaTorneio, Torneio } from '@/types/geral'
...
const SelectTorneio: React.FC<CustomMultiSelectProps> = ({
  id,
  setId,
  setDescricao,
  invalid = false,
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        setLoading(true)
        const response = await apiGeral.getResource<Torneio>('/torneio', {
          search,
          pageSize: 15,
        })
        const registrosData = (response.data ?? []).map((record: Torneio) => ({
          value: record.id,
