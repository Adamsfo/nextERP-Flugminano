...
import { Analise, Matriz, Norma, TorneioItem } from '@/types/geral'
...
const SelectMatriz: React.FC<CustomMultiSelectProps> = ({
  id,
  setId,
  setDescricao,
  laboratorioId,
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        setLoading(true)
        const response = await apiGeral.getResource<Matriz>('/matriz', {
          search,
          filters: { laboratorioId },
          pageSize: 15,
        })
        const registrosData = (response.data ?? []).map((record: Matriz) => ({
          value: record.id,
