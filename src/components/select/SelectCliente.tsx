...
import { Blind, ClienteFornecedor, EstruturaTorneio, Torneio } from '@/types/geral'
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        ...
        const registrosData = (response.data ?? []).map((record: ClienteFornecedor) => ({
          value: record.id,
          label: record.razaoSocialNome,
        }))
