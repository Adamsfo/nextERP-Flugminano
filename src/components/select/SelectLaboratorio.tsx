...
const SelectLaboratorio: React.FC<CustomMultiSelectProps> = ({
  id,
  setId,
  setDescricao,
  invalid = false,
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        setLoading(true)
        const response = await apiGeral.getResource<Laboratorio>('/laboratorio', {
          search,
          pageSize: 15,
        })
        const registrosData = (response.data ?? []).map((record: Laboratorio) => ({
          value: record.id,
          label: record.nome,
        }))
