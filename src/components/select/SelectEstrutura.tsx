...
import { Blind, EstruturaTorneio } from '@/types/geral'
...
const SelectEstrutura: React.FC<CustomMultiSelectProps> = ({
  id,
  setId,
  setDescricao,
  invalid = false,
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        setLoading(true)
        const response = await apiGeral.getResource<EstruturaTorneio>('/estrutura', {
          search,
          pageSize: 15,
        })
...
export default SelectEstrutura
