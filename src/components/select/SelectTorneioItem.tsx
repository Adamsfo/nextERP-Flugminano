...
import { TorneioItem } from '@/types/geral'
...
const SelectTorneioItem: React.FC<CustomMultiSelectProps> = ({
  id,
  setId,
  setDescricao,
  torneioId,
  ...
  const getRegistros = useCallback(
    async (search = '') => {
      ...
      timer = setTimeout(async () => {
        setLoading(true)
        const response = await apiGeral.getResource<TorneioItem>('/torneioitem', {
          search,
          filters: { torneioId },
          pageSize: 15,
        })
        const registrosData = (response.data ?? []).map((record: TorneioItem) => ({
          value: record.id,
...
export default SelectTorneioItem
